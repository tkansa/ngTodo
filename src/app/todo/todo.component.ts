...
export class TodoComponent implements OnInit {
  ...
  completeTodo = function(todo): void {
    let t : Todo = this.todos.find(t => t.task === todo.task);
    t.completed = true;
  }

  filter = function():void {
  
      let filteredTodos = this.todos.filter(t => t.task.includes(this.filterText));
      this.todos = filteredTodos;
    
  }

  removeTodo = function(todo): void{
